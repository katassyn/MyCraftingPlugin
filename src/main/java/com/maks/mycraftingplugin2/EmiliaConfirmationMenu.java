...
public class EmiliaConfirmationMenu {

    public static void open(Player player, int itemId) {
        Inventory inv = Bukkit.createInventory(null, 45, "Emilia Exchange");
    ...
    private static void setRequiredItems(Inventory inv, int itemId) {
        try (Connection conn = Main.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM emilia_items WHERE id = ?")) {
    ...
    private static ItemStack getResultItem(int itemId) {
        try (Connection conn = Main.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT item FROM emilia_items WHERE id = ?")) {
    ...
    private static ItemStack createExchangeButton(int itemId) {
        try (Connection conn = Main.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT cost, daily_limit FROM emilia_items WHERE id = ?")) {
