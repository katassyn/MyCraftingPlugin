...
/**
 * Manages Gnob shop transactions and daily limits.
 */
public class GnobTransactionManager {
    ...
    public static int getTransactionCount(UUID playerUUID, int itemId) {
        ...
        return TransactionManager.getTransactionCount(playerUUID, itemId, "gnob");
    }
    ...
    public static boolean recordTransaction(UUID playerUUID, int itemId) {
        // Use TransactionManager for better timezone handling
        return TransactionManager.recordTransaction(playerUUID, itemId, "gnob");
    }
        ...
        try (Connection conn = Main.getConnection();
            ...
            if (rs.next()) {
                ...
                if (dailyLimit > 0) {
                    ...
                    if (currentCount >= dailyLimit) {
                        ...
                        if (Main.getInstance().getConfig().getInt("debug", 0) == 1) {
                            Bukkit.getLogger().info("[GnobTransaction] Player " + player.getName() +
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    if (requiredItemData != null) {
                        ...
                        if (totalAmount < neededAmount) {
                            if (Main.getInstance().getConfig().getInt("debug", 0) == 1) {
                                Bukkit.getLogger().info("[GnobTransaction] Player missing: " +
                ...
                for (ItemStack requiredItem : requiredItems.values()) {
                    ...
                    if (Main.getInstance().getConfig().getInt("debug", 0) == 1) {
                        Bukkit.getLogger().info("[GnobTransaction] Removed: " +
    ...
    public static void cleanupOldTransactions() {
        // Use TransactionManager for better timezone handling
        TransactionManager.cleanupOldTransactions("gnob");
    }
